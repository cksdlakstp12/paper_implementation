...
def softTeaching_every_batch(s_model: SSD300,
    ...
    for (batch_idx, (wL_image_vis, wL_image_lwir, sL_image_vis, sL_image_lwir, L_boxes, L_labels, L_is_anno)), \
        ...
        sup_box_loss, un_cls_loss, un_loc_loss, sup_box_n_positives = criterion_box(sup_predicted_locs, sup_predicted_scores, un_boxes, un_labels)
        un_loss = calculate_unsupervised_loss(teacher_scores, un_predicted_locs, un_predicted_scores, un_boxes, un_labels)

        # Now compute the losses
        loss = sup_box_loss + 4 * un_loss 
