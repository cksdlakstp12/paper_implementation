...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels, weights=None):
        ...
        if true_locs[positive_priors].shape[0] == 0:
            ...
            weights = weights.unsqueeze(1).expand(-1, n_priors).expand(batch_size, -1)
            selected_weights = weights[positive_priors]
            selected_weights = selected_weights.view(-1, 1).expand(-1, 4)
            loc_loss = F.smooth_l1_loss(predicted_locs[positive_priors].view(-1, 4), true_locs[positive_priors].view(-1, 4), reduction='none')
            loc_loss = loc_loss * selected_weights
            loc_loss = loc_loss.sum() / positive_priors.sum(dim=1).clamp(min=1).sum()
...
class SceneLoss(nn.Module):
    ...
    def forward(self, target, source):
        ...
        for i in range(N):
            ...
            kl_loss = self.kl_loss(self.soft_max(source_i).log(), self.soft_max(target_i)) ## 3x1

            result = torch.cat((result, kl_loss.unsqueeze(0)), dim=0)

        return result.mean() #* 10.0
...
class QualityFocalLoss(nn.Module):
    ...
    def forward(self, preds, teacher_preds, weights=None):
        """
        preds: 학생 모델의 여러 feature map의 예측값, 각 feature map의 shape: [N, C, H, W]
        teacher_preds: 교사 모델의 여러 feature map의 예측값, 각 feature map의 shape: [N, C, H, W]
        weights: 각 feature map의 예측에 대한 가중치, 각 feature map의 shape: [N, C, H, W]
        """
        total_loss = 0.0
        num_features = len(preds)

        if weights is None:
            weights = [torch.ones_like(pred) for pred in preds]

        for pred, teacher_pred, weight in zip(preds, teacher_preds, weights):
            pred_sigmoid = torch.sigmoid(pred)
            teacher_sigmoid = torch.sigmoid(teacher_pred)

            pt = pred_sigmoid
            zerolabel = torch.zeros_like(pt)
            loss = F.binary_cross_entropy(pred_sigmoid, zerolabel, reduction='none') * pt.pow(self.beta)

            if weight is not None:
                pos = weight > 0
                pt = teacher_sigmoid[pos] - pred_sigmoid[pos]
                loss[pos] = F.binary_cross_entropy(pred_sigmoid[pos], teacher_sigmoid[pos], reduction='none') * pt.pow(self.beta)
                
            valid = weight >= 0 if weight is not None else torch.ones_like(loss, dtype=torch.bool)
